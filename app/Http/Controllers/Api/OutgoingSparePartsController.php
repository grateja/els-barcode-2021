...
class OutgoingSparePartsController extends Controller
{
    public function index(Request $request) {
        $result = DB::table('outgoing_spare_part_reports')
            ->leftjoin('clients', 'clients.id', '=', 'outgoing_spare_part_reports.client_id')
            ->leftjoin('subdealers', 'subdealers.id', '=', 'outgoing_spare_part_reports.subdealer_id')
            ->leftjoin('reservations', 'reservations.id', '=', 'outgoing_spare_part_reports.reservation_id')
            ...
            })->selectRaw('*, outgoing_spare_part_reports.id as id, COALESCE(`clients`.`owner_name`, `clients`.`shop_name`) as owner_name, COALESCE(`subdealers`.`subdealer_name`, `subdealers`.`company_name`) as subdealer_name');
    ...
    public function show($reportId) {
        $report = OutgoingSparePartReport::with('client', 'subdealer', 'reservation')->findOrFail($reportId);
    ...
    public function create(Request $request) {
        ...
        if($request->validate($rules)) {
            return DB::transaction(function () use ($request, $reservation) {
                $report = OutgoingSparePartReport::create([
